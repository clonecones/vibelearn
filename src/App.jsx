...
s.textContent=`*{box-sizing:border-box;margin:0;padding:0;} body{-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;} .vl-hover{transition:transform 0.17s ease,box-shadow 0.17s ease;cursor:pointer;} .vl-hover:hover{transform:translateY(-2px);box-shadow:0 12px 36px rgba(0,0,0,0.11)!important;} .vl-btn{transition:all 0.14s ease;cursor:pointer;} .vl-btn:hover:not(:disabled){transform:translateY(-1px);filter:brightness(1.06);} .vl-btn:active:not(:disabled){transform:translateY(0);} .vl-back:hover{color:#6366f1!important;} textarea{font-size:16px!important;} textarea:focus{outline:none!important;border-color:#6366f1!important;box-shadow:0 0 0 3px rgba(99,102,241,0.13)!important;} @keyframes fadeUp{from{opacity:0;transform:translateY(18px)}to{opacity:1;transform:translateY(0)}} @keyframes fadeIn{from{opacity:0}to{opacity:1}} @keyframes popIn{0%{transform:scale(0.8);opacity:0}65%{transform:scale(1.05)}100%{transform:scale(1);opacity:1}} @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.35}} @keyframes slideDown{from{opacity:0;transform:translateY(-8px)}to{opacity:1;transform:translateY(0)}} @keyframes cardIn{from{opacity:0;transform:translateY(12px) scale(0.98)}to{opacity:1;transform:translateY(0) scale(1)}} @keyframes scoreCount{from{opacity:0;transform:scale(0.5)}to{opacity:1;transform:scale(1)}} .vl-fade{animation:fadeUp 0.36s ease both;} .vl-fi{animation:fadeIn 0.26s ease both;} .vl-pop{animation:popIn 0.32s ease both;} .vl-pulse{animation:pulse 1.6s ease infinite;} .vl-slide{animation:slideDown 0.22s ease both;} .vl-card-in{animation:cardIn 0.3s cubic-bezier(.34,1.56,.64,1) both;} .vl-score{animation:scoreCount 0.6s cubic-bezier(.34,1.56,.64,1) both;} .vl-xp{transition:width 1s cubic-bezier(.4,0,.2,1);} .vl-opt{transition:all 0.15s ease;cursor:pointer;} .vl-opt:not(:disabled):hover{transform:translateX(4px);} ::-webkit-scrollbar{width:5px;} ::-webkit-scrollbar-thumb{background:#d4d0c8;border-radius:3px;} .vl-card-body{font-size:16px;line-height:1.85;} @media(min-width:700px){.vl-card-body{font-size:18px!important;line-height:1.95!important;}} @media(max-width:480px){ .vl-topic-card{padding:14px 14px!important;} .vl-topic-title{font-size:16px!important;} }`;
...
// ── DIAGNOSTIC RESULTS ────────────────────────────────────────────────────────
// CHANGE 2: Collapsed post-diagnostic view — score + one CTA above the fold only
...
```
  <div style={{maxWidth:600,margin:"0 auto",padding:"32px 16px"}}>
    {/* Score — above the fold */}
    <div className="vl-fade" style={{background:"#fff",borderRadius:20,padding:"32px 28px",boxShadow:"0 4px 24px rgba(0,0,0,0.06)",border:"1px solid #ebe8e0",marginBottom:16,textAlign:"center"}}>
      <div style={{fontSize:13,color:"#6366f1",fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",marginBottom:12}}>Your Results</div>
      ...
      {/* Recommended track inline — compact */}
      <div style={{background:"#f9f8f5",borderRadius:14,padding:"16px 18px",textAlign:"left",marginBottom:20,display:"flex",alignItems:"center",gap:12}}>
        <div style={{fontSize:32,flexShrink:0}}>{recommendedTrack.emoji}</div>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:12,color:"#9ca3af",fontWeight:700,letterSpacing:1,textTransform:"uppercase",marginBottom:2,...F}}>Recommended track</div>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:18,fontWeight:800,color:"#111827"}}>{recommendedTrack.label}</div>
        </div>
      </div>

      {/* Single primary CTA */}
      <button onClick={()=>onStart(recommendedTrack)} className="vl-btn" style={{background:"#6366f1",color:"#fff",border:"none",borderRadius:14,padding:"16px 28px",fontSize:16,fontWeight:700,...F,width:"100%",boxShadow:"0 4px 16px rgba(99,102,241,0.3)"}}>
    ...
    <div style={{textAlign:"center"}}>
      <button onClick={onExplore} className="vl-btn" style={{background:"transparent",color:"#9ca3af",border:"none",fontSize:14,fontWeight:500,...F,cursor:"pointer"}}>
...
// CHANGE 1: News feed is now a collapsible section, hidden by default
function AINewsFeed(){
...
if(Date.now()-ts<CACHE_TTL){setNews(data);return;}
}
}catch{}
fetchNews();
}
...
const next=!open;
setOpen(next);
if(next&&!news&&!loading)loadNews();
}

return(
<div style={{marginBottom:24}}>
{/* Collapsed toggle row */}
<button onClick={toggle} style={{width:“100%”,background:“none”,border:“none”,cursor:“pointer”,display:“flex”,alignItems:“center”,justifyContent:“space-between”,padding:“10px 0”,…F}}>
<div style={{display:“flex”,alignItems:“center”,gap:8}}>
<div style={{width:8,height:8,borderRadius:999,background:”#ef4444”,boxShadow:“0 0 0 3px rgba(239,68,68,0.2)”,animation:“pulse 2s ease infinite”,flexShrink:0}}/>
<span style={{fontSize:13,color:”#374151”,fontWeight:700,letterSpacing:1.5,textTransform:“uppercase”}}>AI News — Last 24 Hours</span>
</div>
<span style={{fontSize:13,color:”#9ca3af”,fontWeight:600,transition:“transform 0.2s”,display:“inline-block”,transform:open?“rotate(180deg)”:“rotate(0deg)”}}>▾</span>
</button>

```
  {open&&(
    <div className="vl-slide">
      {loading&&(
...
```
  {/* Header — score pill only, no duplicate stats */}
  <div style={{background:"#fff",borderBottom:"1px solid #ebe8e0",padding:"14px 20px",position:"sticky",top:0,zIndex:100}}>
    <div style={{maxWidth:880,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
      ...
      {/* CHANGE 3: Score pill in header only — stats strip removed */}
  ...
  <div style={{maxWidth:880,margin:"0 auto",padding:"20px 16px"}}>
    ...
    {/* CHANGE 6: Retake diagnostic — quiet text link only, no tile */}
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
      {/* Progress bar */}
      <div style={{flex:1,marginRight:16}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          <span style={{fontSize:13,color:"#6b7280",fontWeight:600,...F}}>Overall progress</span>
          <span style={{fontSize:13,color:"#6366f1",fontWeight:700,...F}}>{st.completed.length}/{TOPICS.length} · {Math.round(st.completed.length/TOPICS.length*100)}%</span>
        </div>
      ...
      <button onClick={()=>setView("diagnostic")} className="vl-btn" style={{background:"none",border:"none",color:"#9ca3af",fontSize:13,cursor:"pointer",...F,padding:0,whiteSpace:"nowrap",flexShrink:0}}>
    ...
    {st.completed.length<TOPICS.length&&(()=>{
            ...
            <div style={{background:"#fff",borderRadius:18,border:`2px solid ${userTrack.color}`,overflow:"hidden",boxShadow:"0 4px 20px rgba(0,0,0,0.06)"}}>
              ...
              <div onClick={()=>{setFromTrack(userTrack);openTopic(nextTrackTopic);}} style={{cursor:"pointer"}}>
                <div style={{padding:"16px 20px",display:"flex",alignItems:"center",gap:14}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    {/* CHANGE 5: Personalization line */}
                    <div style={{fontSize:13,color:"#6b7280",fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:2,...F}}>Up next · Based on your diagnostic</div>
                ...
                {/* CHANGE 7: "Continue learning" */}
                <div style={{background:userTrack.color,padding:"12px 20px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <span style={{fontSize:14,fontWeight:700,color:"#fff",...F}}>Continue learning</span>
              ...
              <div style={{borderTop:"1px solid #f3f4f6",padding:"10px 20px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                {/* CHANGE 4: Tracks moved here — link only */}
      ...
      if(recommended){return(
        <div style={{marginBottom:20}} className="vl-fade">
          {/* CHANGE 7: "Your next lesson" */}
          <div style={{fontSize:13,color:"#6b7280",fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",marginBottom:10,...F}}>
            {userTrack&&trackComplete?"Track complete! Up next":"Your next lesson"}
          </div>
          <div onClick={()=>openTopic(recommended)} style={{cursor:"pointer",borderRadius:20,overflow:"hidden",boxShadow:"0 8px 28px rgba(99,102,241,0.32)",border:"none"}}>
            <div style={{background:"linear-gradient(135deg,#6366f1 0%,#818cf8 100%)",padding:"20px 22px",display:"flex",alignItems:"center",gap:16}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                {/* CHANGE 5: Personalization line when diagnostic done */}
                <div style={{fontSize:13,color:"rgba(255,255,255,0.75)",fontWeight:600,letterSpacing:0.5,marginBottom:4,...F}}>
                  {st.diagnosticDone?"Based on your diagnostic":"Up next for you"}
                </div>
            ...
            <div style={{background:"#4f46e5",padding:"14px 22px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <span style={{fontSize:15,fontWeight:700,color:"#fff",...F}}>Continue learning</span>
    ...
    {/* CHANGE 1: News feed below All Topics, collapsed by default */}
    <AINewsFeed/>

    {/* CHANGE 4: Skill Tracks section — below news feed */}
    <div style={{marginBottom:24}}>
      <button onClick={()=>setView("tracks")} className="vl-btn" style={{width:"100%",background:"#fff",border:"1.5px solid #ebe8e0",borderRadius:16,padding:"16px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <span style={{fontSize:20}}>🗺️</span>
          <div style={{textAlign:"left"}}>
            <div style={{fontSize:15,fontWeight:700,color:"#111827",...F}}>Skill Tracks</div>
            <div style={{fontSize:13,color:"#6b7280",...F}}>
              {TRACKS.filter(t=>t.slugs.every(s=>st.completed.includes(s))).length}/{TRACKS.length} tracks complete
            </div>
          </div>
        </div>
